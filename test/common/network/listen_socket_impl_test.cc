...
TEST(ListenSocket, All) {
  ...
  TcpListenSocket socket2("tcp://127.0.0.1:15000", true);
  listen(socket2.fd(), 0);
  EXPECT_EQ("127.0.0.1:15000", socket2.localAddress()->asString());

  EXPECT_THROW(Network::TcpListenSocket socket3(uint32_t(15000), true), EnvoyException);
  EXPECT_THROW(Network::TcpListenSocket socket4("tcp://127.0.0.1:15000", true), EnvoyException);
